...
// DeploymentEnvironment provides common setup for writing tests using FAST
...
// NewDeploymentEnvironment creates a FunctionalEnvironment with a basic setup for writing tests using the FAST library.
...
func (env *DeploymentEnvironment) TeardownProcess(ctx context.Context, p *fast.Filecoin) error {
	return env.TeardownProcess(ctx, p)
}
